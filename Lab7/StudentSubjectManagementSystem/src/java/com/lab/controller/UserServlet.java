        ...
        if ("register".equals(action)) {
            ...
            if (isSuccess) {
                response.getWriter().println(
                    "<h3>Registration Successful! <a href='" + request.getContextPath() + "/login.html'>Login here</a></h3>"
                );
            } else {
                response.getWriter().println(
                    "<h3>Registration Failed! <a href='" + request.getContextPath() + "/register.html'>Try Again</a></h3>"
                );
            }
            ...
            if (student != null) {
                ...
                session.setAttribute("loggedUser", student); // ✅ Match case pattern
                response.sendRedirect(request.getContextPath() + "/dashboard.jsp");
            } else {
                response.getWriter().println(
                    "<h3>Invalid Credentials! <a href='" + request.getContextPath() + "/login.html'>Try Again</a></h3>"
                );
            }
        }
    }
